...
function generateOptionsFile(jsonPath, outputPath) {
    ...
    const objToString = obj =>
        Object.entries(obj)
            .map(([key, val]) => {
                // se a chave tiver espaço ou hífen, põe aspas simples
                const keyFormatted = /^[a-zA-Z0-9_]+$/.test(key) ? key : `'${key}'`;
                return `    ${keyFormatted}: "${val.trim()}"`;
            })
            .join(',\n');
    ...
    const arquivoFinal = optionsUseStr + optionsProblemsStr + exportStr;

    fs.writeFileSync(outputPath, arquivoFinal, 'utf-8');
